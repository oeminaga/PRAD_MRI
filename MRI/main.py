...
###########
#
#   Generate Patches and Store them in A Numpy File --> Will be moved to utils sections
#
###########

def multi_slice_viewer(volume, reference):
...
def PrepareData(image, reference_shape=(int(round(1.535*512)),int(round(1.535*512))), window_size=(256,256), verbose=False):
    ct_scan = sitk.GetArrayFromImage(image)
    ct_scan_resized = np.zeros((ct_scan.shape[0], window_size[0], window_size[1]))
    for index, img in enumerate(ct_scan):
        img_copy = img.copy()
        if verbose:
            plt.imshow(img_copy)
        ...
        if verbose:
            plt.imshow(sharpened)
            plt.show()
        img_copy = exposure.equalize_adapthist(sharpened, clip_limit=0.05, nbins=1000)
        
        
        if verbose:
            print(np.mean(img_copy))
            plt.imshow(sharpened)
            plt.show()
        #Resize if small
        img_copy = Image.fromarray(img_copy)
        
        if img.shape is not reference_shape:
            img_copy = img_copy.resize(reference_shape, resample=PIL.Image.LANCZOS)
...
def Run(args):
    ...
    if not args.testing:
        ...
        eval_model.summary()
        test(args, model=eval_model)#"/home/eminaga/EncryptedData/Challenge/MoNuSeg Training Data/All/")
 
def train(args, model, verbose=False):
    #MRI detection is very senstive to augmentation and transformation. Lots papers avoided shear, brightness and rotation. Select carefuly!
    ...
    seed = 1
    if (args.load_numpy):
        #Only for internal test and training. Please run first main.py 
        # -gp in order to generate the required files. Please  
        #specify the location of test and training set.
    ...
    #Test different weights after 10 epochs and see if there is improvment in accuracy during training.  Weight changing during training. reset the weight if needed.
...
def test(args, model, verbose=False):
    ...
    print('*****'*30)
    print(args)
    print('Onlive',args.Onlive)
    Onlive=args.Onlive
    if Onlive:
        seg = SegmentProstate(args, model)
        np.save(seg, args.store_name)
    else:
        #Only for internal test.
        test_img_dataset = np.load('./img_test_set.npy')
        test_mask_dataset = np.load('./mask_test_set.npy')
        
        from sklearn.metrics import confusion_matrix,classification_report
        report_list = []
        report_list_one_hot = []
        with progressbar.ProgressBar(max_value=test_img_dataset.shape[0]) as bar:    
            for i, img in enumerate(test_img_dataset):
                mask = test_mask_dataset[i]
                img_ = img.reshape(1, img.shape[0], img.shape[1],1)
                x_x,heatmap_predicted_y_x, _ = model.predict(img_)
                if verbose:
                ...
                if verbose:
                    print(x_x.shape)
                mask_x = np.logical_or(x_x_01==1, x_x_02==1)
                if verbose:
                    plt.imshow(mask_x)
                    plt.show()
                    ...
                    plt.imshow(mask[:,:,1])
                    plt.show()
                heat_pred = heatmap_predicted_y_x[0,:,:,0] > 0.5
                if verbose:
                    plt.imshow(heat_pred==mask[:,:,1])
                    plt.show()
                    plt.imshow(img_[0,:,:,0])
                    plt.show()
                y_pred = heatmap_predicted_y_x[:].flatten() > 0.5
                rtx= mask_x.flatten()
                y_true_01 = mask[...,1].flatten()>0.5
                y_true_02 = mask[...,2].flatten()>0.5
                y_true = np.logical_or(y_true_01==1, y_true_02==1)
                dr = confusion_matrix(y_true, rtx)
                report = classification_report(y_true, rtx, output_dict=True)#, target_names=class_labels)
        ...
        SaveTheResult(report_list_one_hot, './report_f1score_all.pkl')
        print('F1 of prostate area, seg of prostate and contour areas')
        print('mean',np.mean(np.array(report_list_one_hot)))
        print('median',np.median(np.array(report_list_one_hot)))
        print('mean,[95% CI]:',mean_confidence_interval(report_list_one_hot))
        SaveTheResult(report_list, './report.pkl')
        print('F1 of prostate area, only seg area of prostate')
        print('mean',np.mean(np.array(report_list)))
        print('median',np.median(np.array(report_list)))
        print('mean,[95% CI]:',mean_confidence_interval(report_list))
    print('*****'*30)
...
######
#
# Call and Generate Segmentation File (Numpy format) for T2-weighted MRI
#
######
def SegmentProstate(args, model, window_size=(256,256), output_channel=3):
    mri_seq =ReadImages(path=args.dicom)
    npy_mri_seq = PrepareData(mri_seq, window_size=window_size)
    prostate_segmentation = np.zeros((npy_mri_seq.shape[0], window_size[0], window_size[1],output_channel)) 
    for i, img in enumerate(npy_mri_seq):
        img_ = img.reshape(1, img.shape[0], img.shape[1],1)
        heatmap_predicted_y_x,_, _ = model.predict(img_)
        nbr_channel = heatmap_predicted_y_x.shape[2]
        prostate_segmentation[i,:,:,0:nbr_channel] = heatmap_predicted_y_x[0,:,:,0:nbr_channel]
        '''
        plt.imshow(heatmap_predicted_y_x[0,:,:,0:nbr_channel])
        plt.show()
        plt.imshow(img_[0,:,:,0])
        plt.show()
        '''
    segx = npy_mri_seq
    #segx = prostate_segmentation[...,0] * segx
    multi_slice_viewer(prostate_segmentation, npy_mri_seq)
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dicom', help='define the path that includes the dicom files', default='./MR series/X_X_Ax_T2_smFOV_prostate/')
    parser.add_argument('--store_name', help='give the file name for storing the segmentation result as numpy', default='./segmentation.npy')
    parser.add_argument("--path", help="the path where the train data are stored",
                    default="./train/")  
    parser.add_argument("--test", help="the path where the test data are stored",
                    default="./test/")         
    ...
    parser.add_argument('-ol', '--Onlive', default=True, action='store_true',
                    help="Run the seqmentation direclty.")
    ...
    #Use -gp to generate the training, validation and test set from the given folder
    #Use -dicom, -store_name -Onlive to run your own MRI images. Only one case!
    #Example main.py -dicom ./X_X_MRI/ -Onlive -store_name ./X_X_MRI.npy
    #The code may inherit some bugs, please use with cautious